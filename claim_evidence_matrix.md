...
| Invalid coupling leaves the posterior mean unchanged | The gate product in the bounded-coupling operator becomes zero | Optional coupling operator | This is a coupling fallback; regional candidate-pool restarts may still change the trajectory. |
| Main Chen2020 runs do not rely on a posterior-mean shift | Archived configuration has region-lift override disabled and no accepted mean shift | Main Chen2020 comparison | The principal result evaluates warm start plus early candidate-pool expansion under ordinary EI. |
...
| Regional coupling has not been independently established | Complete \(0.3900\pm0.0067\) is essentially level with warm start alone; regional archive has a restart confound | Component archive only | Do not claim additivity, synergy, or a systematic regional benefit. |
