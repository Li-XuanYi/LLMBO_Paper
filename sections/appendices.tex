...
Seed-level identifiers, LLM call records, and parsed responses are retained in
the experiment logs rather than in the main parameter table.
The principal configuration is summarized below so that the reported statistics
can be traced to the corresponding seed set without interrupting the main
experimental narrative.

\begin{table}[!t]
\caption{Seed and Configuration Summary for the Reported Studies}
...
Study & Configuration \\
\midrule
Chen2020 main comparison & five seeds (\MainSeedSet); 56 calls per seed \\
Ecker2015 second-parameterization case & five seeds (\MainSeedSet); 56 calls per seed \\
Initialization study & ten matched seeds; 26-call budget \\
Component ablation & three matched seeds (8409--8411); 16-call budget \\
Main regional guidance & $T_{\mathrm L}=16$, 64 anchors, EI-search restarts \\
LLM records & prompt/configuration and responses logged per seed \\
\bottomrule
\end{tabular}
\end{table}
