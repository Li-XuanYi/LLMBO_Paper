...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.72\textwidth]{optimal_protocols_curve.pdf}
\caption{Chen2020 Pareto-archive growth over 50 algorithmic iterations. Lines
are five-run means and bands show across-run dispersion.}
...
retained set of trade-offs in these runs.
...
no normalization on Chen2020 over five runs and the same 56-evaluation budget.
It tests numerical robustness rather than the LLM mechanism.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.72\textwidth]{objective_normalization_hv.pdf}
\caption{Chen2020 objective-preprocessing sensitivity over five runs. Lines
...
\section{Run Configuration and Reproducibility}
\label{app:reproducibility}

The main Chen2020 and Ecker2015 comparisons use five runs for each method.
Run-level identifiers, random seeds, LLM call records, and parsed responses are
retained in the experiment logs rather than in the main parameter table.
The principal manuscript-facing configuration is summarized below so that the
reported figures can be traced to the archived runs without interrupting the
main experimental narrative.

\begin{table}[!t]
\caption{Run-Level Configuration Used in the Reported Studies}
\label{tab:run_configuration}
\centering
\footnotesize
...
Study & Configuration \\
\midrule
Chen2020 main comparison & five runs; 56 simulator calls per run \\
Ecker2015 robustness case & five runs; 56 simulator calls per run \\
Initialization study & ten matched runs; 26-call budget \\
Component ablation & three matched runs; 16-call budget \\
Main regional guidance & $T_{\mathrm L}=16$, 64 anchors, EI-search restarts \\
LLM records & prompt/configuration and responses logged per run \\
\bottomrule
\end{tabular}
\end{table}

For the main algorithm, regional information affects only acquisition-search
restarts. The posterior-mean coupling in Appendix~\ref{app:coupling} is a
separate diagnostic realization and is not used to define the principal results.
